...
def get_sphere(radius: float = 1, thickness: float = -1):
    ...
    '''
    if bpy is None:
        logger.warning("Blender (bpy) is not installed. Mesh generation skipped.")
        return None
...
def export(mesh, output_path: str):
    ...
    '''
    if bpy is None:
        logger.warning("Blender (bpy) is not installed. Export skipped.")
        return
    if mesh is None:
        raise ValueError("Cannot export mesh: mesh is None.")
    if isinstance(output_path, pathlib.Path):
        output_path = str(output_path)

    # Ensure the output directory exists and use a normalized absolute path.
    ...
    # Blender >= 4.x generally provides wm.stl_export.
    # Try multiple compatible signatures before falling back.
    try:
        ...
        logger.debug("wm.stl_export(selected) failed: %s", str(e1))
    ...
    try:
        ...
        logger.debug("wm.stl_export(default) failed: %s", str(e2))

    # Legacy fallback (older Blender versions only).
    if hasattr(bpy.ops, "export_mesh") and hasattr(bpy.ops.export_mesh, "stl"):
        bpy.ops.export_mesh.stl(filepath=output_path, check_existing=False)
        return

    raise RuntimeError(
        f"STL export failed for '{output_path}'. No compatible Blender STL exporter available."
    )
